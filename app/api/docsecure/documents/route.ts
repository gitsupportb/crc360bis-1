...
function callPythonScript(
  ...
  return new Promise((resolve) => {
    // Try to call Python script, fallback to mock data if it fails
    const testScriptPath = path.join(process.cwd(), 'utils', 'docsecure', 'test_integration.py');

    const python = spawn('python3', [testScriptPath, action], {
      cwd: process.cwd(),
    ...
    python.on('close', (code) => {
      ...
      if (params.search) {
        ...
        resolve({
          success: true,
          message: `Document with ID ${params.documentId} deleted successfully (mock)`
        });
      }
    });
