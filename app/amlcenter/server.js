...
// Home route - both root and /amlcenter
app.get('/', (req, res) => {
...
// Upload Excel and match route
app.post('/match-excel', upload.single('excelFile'), (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).send('No file uploaded');
    }
...
// Client Space route
app.get('/client-space', (req, res) => {
  res.render('client-space', { clientData, clientCount });
});

// Function to execute Python script and process its output
...
// Upload Risk Assessment Excel file route
app.post('/upload-risk-assessment', upload.single('excelFile'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).send('No file uploaded');
    }
    ...
    const processedData = await executePythonScript(excelFilePath);
    
    if (!processedData || !processedData.clients) {
      console.error('Error: Python script did not return valid client data');
    ...
    if (clientData && clientData.length > 0) {
      clientData.forEach(client => {
        ...
        // Only use Python-processed data to avoid redundancies
        // If processedRiskTable is missing, create a minimal structure
        if (!client.processedRiskTable || client.processedRiskTable.length === 0) {
          console.log(`No processedRiskTable found for ${client.name}, creating minimal structure`);

          // Create minimal structure with known categories but no factors
          client.processedRiskTable = knownCategories.map(categoryName => ({
            name: categoryName,
            rating: 'Faible',
            factors: [{
              name: 'Données non disponibles',
              profile: 'Aucune donnée extraite du fichier Excel',
              rating: 'Faible'
            }]
          }));
        }

        // Validate and clean the processedRiskTable to ensure no duplicates
        if (client.processedRiskTable && Array.isArray(client.processedRiskTable)) {
          const seenCategories = new Set();
          client.processedRiskTable = client.processedRiskTable.filter(category => {
            if (seenCategories.has(category.name)) {
              console.log(`Removing duplicate category: ${category.name} for client ${client.name}`);
              return false;
            }
            seenCategories.add(category.name);

            // Also clean factors within each category
            if (category.factors && Array.isArray(category.factors)) {
              const seenFactors = new Set();
              category.factors = category.factors.filter(factor => {
                const factorKey = `${factor.name}:${factor.profile}`;
                if (seenFactors.has(factorKey)) {
                  console.log(`Removing duplicate factor: ${factor.name} in category ${category.name}`);
                  return false;
                }
    ...
    // Note: Removed sample risk factors fallback to prevent redundancies
    // The system now relies solely on Python-processed data from Excel files
    
    res.redirect('/client-space');
  } catch (error) {
    console.error('Error processing Risk Assessment Excel:', error);
    res.status(500).send('Error processing Risk Assessment Excel');
  }
});
...
function processRiskAssessmentExcel(workbook) {
  ...
  workbook.SheetNames.forEach(sheetName => {
    ...
    // Note: Disabled server-side processRiskTable to prevent redundancies
    // Now using Python script exclusively for risk table processing
    const processedRiskTable = null; // Will be populated by Python script
    console.log(`Server-side risk table processing disabled for sheet ${sheetName} - using Python script instead`);
    ...
    const client = {
      ...
      extractedRiskData: [],
      // processedRiskTable will be populated by Python script
      processedRiskTable: null
    };
...
// Dedicated function to process risk table (A8:E25) from Excel sheets
function processRiskTable(sheet) {
  // Get the range from a configuration variable instead of hardcoding
  const riskTableRange = 'A8:E25'; // This could be moved to a config object/file
  
  // Extract raw data from the specific range
  const rangeData = extractSpecificRange(sheet, riskTableRange);
  ...
  // Handle empty range data
  if (!rangeData || rangeData.length === 0) {
    console.log('No range data found, creating default category');
    return [{
      name: 'Données non disponibles',
      rating: 'Faible',
      factors: [{
  ...
  // Process each row from the range data
  rangeData.forEach(row => {
    ...
    if (row.isCategory) {
      ...
      processedData.push(currentCategory);
    } 
    // If not a category and we have a current category, it's a risk factor
    else if (currentCategory && row.A) {
      // Combine columns B, C, and D for the profile
      const profileParts = [];
      ['B', 'C', 'D'].forEach(col => {
        if (row[col] && row[col] !== '' && !['Faible', 'Moyen', 'Élevé', 'Elevé', 'None', 'nan'].includes(row[col])) {
          // Try to filter out numeric codes or dates
          const numVal = parseFloat(row[col]);
          if (isNaN(numVal) || numVal <= 10000) {
            profileParts.push(row[col]);
          }
        }
      });
      ...
      // Add this factor to the current category
  ...
  if (processedData.length === 0) {
    processedData.push({
      ...
      factors: [{
        ...
        profile: 'Aucune donnée catégorisée trouvée',
        rating: 'Faible'
      }]
    });
  }
...
const porte = 3001; // ✅ Port fixe - Running on 3001 to avoid conflict with main dashboard

app.listen(porte, () => {
  console.log(`✅ LBCFT WEBAPP en cours d'exécution sur : http://localhost:${porte}`);
});