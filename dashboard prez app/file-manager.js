...
class FileManager {
  ...
  generateFileBrowserHTML() {
    ...
    for (const [regulator, categories] of Object.entries(this.fileStructure)) {
      ...
      html += `<h3 class="regulator-header" onclick="toggleRegulator('${regulatorId}')">
        ...
        <span class="regulator-title">🏛️ ${this.formatRegulatorName(regulator)}</span>
  ...
  formatRegulatorName(regulator) {
    const regulatorNames = {
      ...
      'DGI': '🏛️ Direction Générale des Impôts (DGI)'
    };
    return regulatorNames[regulator] || `🏛️ ${regulator}`;
  }
  ...
  formatCategoryName(category) {
    const categoryNames = {
      ...
      'III___Etats_relatifs_à_la_réglementation_prudentielle': '⚖️ III – Etats relatifs à la réglementation prudentielle',
