...
app.post('/upload-risk-assessment', upload.single('excelFile'), async (req, res) => {
  try {
      ...
      clientData.forEach(client => {
        ...
        if (!client.processedRiskTable || client.processedRiskTable.length === 0) {
          ...
          if (client.extractedRiskData && client.extractedRiskData.length > 0) {
            client.processedRiskTable = processRiskTable({
              Sheets: {
                [client.name]: {}
              }
            });
    ...
    // Add sample risk factors if none were found
    if (clientData && clientData.length > 0) {
      clientData.forEach(client => {
        if (!client.riskFactors || client.riskFactors.length === 0) {
          console.log('Adding sample risk factors for client:', client.name);
          client.riskFactors = [
            {
            ...
            {
              category: 'Caractéristiques du client',
              rating: 'Faible',
              factors: [
                {
                  category: 'Caractéristiques du client',
            ...
            {
              category: 'Nature produits/opérations',
              rating: 'Faible',
              factors: [
                {
                  category: 'Nature produits/opérations',
            ...
            {
              category: 'Canal de distribution',
              rating: 'Faible',
              factors: [
                {
                  category: 'Canal de distribution',
...
const porte = 3000; // ✅ Port fixe

// Create a router for /amlcenter routes
const amlRouter = express.Router();

// Copy all existing routes to the router
amlRouter.get('/', (req, res) => {
  const perPage = parseInt(req.query.perPage) || 20;
  const displayData = pdfData && perPage !== -1 ? pdfData.slice(0, perPage) : pdfData;
  res.render('index', { pdfData: displayData, allPdfData: pdfData, perPage });
});

// Mount all the existing routes on the amlRouter
// We'll need to copy all the routes here

// Mount the amlcenter router
app.use('/amlcenter', amlRouter);

app.listen(porte, () => {
  console.log(`✅ LBCFT WEBAPP en cours d'exécution sur : http://localhost:${porte}`);
  console.log(`✅ AML Center available at: http://localhost:${porte}/amlcenter`);
});